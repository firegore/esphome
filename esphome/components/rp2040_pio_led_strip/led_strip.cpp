...
void RP2040PIOLEDStripLightOutput::write_state(light::LightState *state) {
  ...
  for (int i = 0; i < this->num_leds_; i++) {
    uint8_t c1 = this->is_rgbw_ ? this->buf_[(i * 4) + 0] : this->buf_[(i * 3) + 0];
    uint8_t c2 = this->is_rgbw_ ? this->buf_[(i * 4) + 1] : this->buf_[(i * 3) + 1];
    uint8_t c3 = this->is_rgbw_ ? this->buf_[(i * 4) + 2] : this->buf_[(i * 3) + 2];
